...
check("MODE labeled", /MODE/i.test(mode ?? ""));
check("TAP labeled", /TAP/i.test(tap ?? ""));

const fs1Box = await page.locator(".hx-xl-fs1").boundingBox();
const fs4Box = await page.locator(".hx-xl-fs4").boundingBox();
...
await page.locator("select#device").selectOption("hx-stomp");
...
await page.locator("select#device").selectOption("hx-stomp-xl");
await page.waitForTimeout(700);
...
const assignH = await page.locator("h2").filter({ hasText: "Switch 1" }).count();
check("replica tap selects Switch 1 in assign panel", assignH >= 1);

await page.getByRole("button", { name: "Lead", exact: true }).click();
await page.waitForTimeout(200);
const switch1Label = await switch1.textContent();
check("assigning Lead updates switch 1", /LEAD/i.test(switch1Label ?? ""), switch1Label ?? "");

await page.getByRole("button", { name: "Reset to original" }).click();
await page.waitForTimeout(300);
const restored = await switch1.textContent();
check("reset restores original", /INTRO/i.test(restored ?? ""), restored ?? "");

// Search
await page.getByLabel("Search songs and catalog").first().fill("sandman");
await page.waitForTimeout(300);
const songHit = page.getByRole("button", { name: /Enter Sandman/i });
check("header search lists the song", (await songHit.count()) > 0);
await page.getByLabel("Search songs and catalog").first().fill("");
