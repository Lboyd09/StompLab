...
import { CATEGORIES, CATEGORY_MAP, STOMP_DEVICES } from "@/data/categories";
import { MODEL_MAP, catalogModels, findEquivalents } from "@/data/catalog";
import { isStompModelId, type CategoryId, type StompModelId } from "@/data/types";
...
type Search = { q: string; cat: string; tab: string; unit: string };
...
export const Route = createFileRoute("/catalog/")({
  validateSearch: (s: Record<string, unknown>): Search => ({
    ...
    unit: typeof s.unit === "string" && isStompModelId(s.unit) ? s.unit : "",
  }),
...
function CatalogPage() {
  ...
  const unit = (search.unit as StompModelId | "") || "";
  ...
  return (
    <div className="space-y-6" data-tutorial="catalog">
      ...
      <PageHeader kicker="Every factory model" title="Line 6 catalog">
        Each amp, cab, and pedal on its own page — filtered by HX Stomp, XL, Helix, LT, HX Effects, or POD Go.
        Type a pedal you own and we’ll show the Line 6 name.
      </PageHeader>

      <div className="flex w-fit rounded-full bg-secondary p-1">
        <button
          type="button"
        ...
        <div className="mx-auto max-w-2xl space-y-6">
          <form onSubmit={onAsk} className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Instant matches as you type. Sign in to have us explain the closest Line 6 model.
            </p>
          </form>
        ...
        <>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            <CatChip
              active={!unit}
              label="All units"
              onClick={() => setSearch({ search: (p) => ({ ...p, unit: "" }) })}
            />
            {STOMP_DEVICES.map((d) => (
              <CatChip
                key={d.id}
                ...
                label={d.short}
                onClick={() =>
                  setSearch({ search: (p) => ({ ...p, unit: p.unit === d.id ? "" : d.id }) })
                }
              />
            ))}
          </div>
          ...
          <p className="text-xs tabular-nums text-muted-foreground">
            ...
            {unit ? ` on ${STOMP_DEVICES.find((d) => d.id === unit)?.name ?? unit}` : ""}
          ...
          {!models.length ? (
            <p className="text-sm text-muted-foreground">
              ...
              <button
                ...
                onClick={() => {
                  setLocalQ("");
                  void setSearch({ search: (p) => ({ ...p, q: "", cat: "", unit: "" }) });
                }}
              >
                Clear it
              </button>
